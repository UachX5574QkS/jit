#!/usr/bin/env node
/**
 * Deploy built React assets to ORDS tpo_static_files table.
 * 
 * Uses SQLcl to upload files as base64-encoded BLOBs in chunks.
 * Run: node deploy-to-ords.js
 * 
...
function generateSql(files) {
  ...
  sql += `DELETE FROM tpo_static_files;\nCOMMIT;\n\n`;
  ...
  for (const file of files) {
    ...
    sql += `    INSERT INTO tpo_static_files (file_path, content_type, file_content)\n`;
  ...
  sql += `SELECT file_path, content_type, DBMS_LOB.GETLENGTH(file_content) as size_bytes FROM tpo_static_files ORDER BY file_path;\n`;
  sql += `EXIT;\n`;
  return sql;
}
