> Oracle target variant. This document mirrors `tasks.md` but targets Oracle Autonomous Database + ORDS + IDCS as the sole platform. The non-Oracle original remains authoritative for the current build. The checkbox states below mirror the original plan for the equivalent Oracle build.
...
- [x] 1. Project scaffolding and runtime
  ...
  - [x] 1.2 Create the ORDS-based backend against the Oracle ADB `helpdesk` schema: an ORDS-enabled schema with a REST module skeleton, config for the ADB connection, and a dev script that serves the frontend against the ORDS API.
    - _Requirements: R22.2, R22.3, R22.4_
  - [x] 1.3 Add a parameterised PL/SQL / bind-variable data-access layer (no string-interpolated SQL) and a schema migration approach for the ADB.
    - _Requirements: R22.2, R22.4_

- [x] 2. Database schema and migrations
  - [x] 2.1 Create identity/org tables: `app_user`, `area_manager`, `admin_group` (all timestamps `TIMESTAMP WITH TIME ZONE`).
  ...
  - [x] 2.7 Add the `status` domain as a `VARCHAR2` with a CHECK constraint (or lookup table) and stop-state definitions.
    - _Requirements: R9.1, R9.3_

- [x] 3. Cross-cutting backend services
  - [x] 3.1 Implement password hashing helpers (a one-way hash; the specific KDF is an implementation detail); ensure hashes are never returned or logged.
    - _Requirements: R1.5_
  - [x] 3.2 Implement the `CurrentUser` resolver (IDCS-resolved identity, with the seeded-user testing login against the ADB as a testing convenience) exposing id, roles, teams led, teams member of, isAdmin, timezone.
...
- [x] 4. Authentication endpoints and testing login
  - [x] 4.1 Implement `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me` resolving the IDCS-authenticated CurrentUser (with the seeded-user testing login as a convenience).
    - _Requirements: R1.1, R1.3, R1.4, R1.7_
  - [x] 4.2 Implement `GET /api/auth/users` (testing convenience) for the login drop-down formatting.
    - _Requirements: R1.2_
...
- [x] 6. Request lifecycle APIs
  ...
  - [x] 6.2 `POST /api/review/summary` calling Ollama server-side from the ORDS/PL/SQL handler (outbound HTTP from the database) with short timeout and `{available:false}` fallback; display-only, not persisted.
...
- [x] 10. Login and New workflow (frontend)
  - [x] 10.1 Login screen with seeded-user testing drop-down; establish session; load CurrentUser.
