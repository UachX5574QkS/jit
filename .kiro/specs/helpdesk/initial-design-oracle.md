> Oracle target variant. This document mirrors `initial-design.md` but targets Oracle Autonomous Database + ORDS + IDCS as the sole platform. The non-Oracle original remains authoritative for the current build.
...
The backend data store is an Oracle Autonomous Database (ADB) — the `helpdesk` schema within the Autonomous Database. The `helpdesk` ADB schema is provisioned and reachable for schema creation, seed data, and queries.

## Runtime

The tool runs against Oracle end to end. The backend is implemented as Oracle REST Data Services (ORDS) REST endpoints running inside the Autonomous Database, exposing the API to the Angular frontend over HTTPS/JSON. The built Angular static files are served via ORDS from the Autonomous Database.

- **Static files** (the built Angular app) served via Oracle REST Data Services (ORDS).
- **Authentication** delegated to Oracle Identity Cloud Service (IDCS).

The design keeps the frontend and its data-access concerns loosely coupled from the ORDS/PL/SQL implementation, so the frontend talks only to a documented REST contract and treats "who is the current user" as a single resolved identity.
...
Authentication is delegated to IDCS. For the benefit of local testing against the ADB, a convenience login is provided where the username is a drop down of each person created and their surname set to their role, for example 11111111 Jason (Administrator) Hughes, 22222222 John (Support) Smith.

This seeded-user drop-down login is a testing convenience only; the authoritative authentication source is IDCS. The application treats "who is the current user" as a single resolved identity so the auth source (IDCS in normal operation, the seeded-user login for local testing) can be swapped without touching the rest of the app.
...
1. User is presented with a summary of the request. This can be obtained by accessing the locally running Ollama model which is running as a service, called server-side from the ORDS/PL/SQL handler (for example via an outbound HTTP call from the database). If the service is unavailable then display the values entered by the user.
