> Oracle target variant. This document mirrors `design.md` but targets Oracle Autonomous Database + ORDS + IDCS as the sole platform. The non-Oracle original remains authoritative for the current build.

# Design

## Overview

The Helpdesk tool is a browser-based single-page application built with Angular (latest) and styled per the `ui-foundations` steering specification. It is backed by an Oracle Autonomous Database (ADB) (the `helpdesk` schema within the ADB) reached through ORDS REST endpoints that run inside the Autonomous Database. The built static files are served via Oracle ORDS from the Autonomous Database, and authentication is delegated to IDCS.

This design deliberately isolates two things behind abstractions so feature code stays independent of the platform substrate:

1. **Identity** — a single `CurrentUser` resolution point, so IDCS authentication (or the seeded-user testing login) is resolved to one identity without touching feature code.
2. **Data access** — feature code talks to typed Angular services over a documented REST contract, never to the database directly, so the ORDS/PL/SQL implementation can evolve independently.
...
```
Browser (Angular SPA, served via ORDS)
   │  HTTPS/JSON  (typed Angular HttpClient services)
   ▼
ORDS REST  (in the Autonomous Database)
   │  PL/SQL handlers, parameterised SQL (bind variables)
   ▼
Oracle Autonomous Database  (helpdesk schema)
   │
Ollama service ← called from the ORDS/PL/SQL handler (outbound HTTP from the DB) for the Step 3 summary
```
...
- **Backend API**: ORDS REST endpoints (an ORDS-enabled schema exposing RESTful services / PL/SQL handlers) that own authentication, authorisation, validation, the status state machine, timer logic, audit writes, statistics aggregation, and the Ollama call. The API is implemented as ORDS handlers running inside the Autonomous Database _(R22.3)_.
- **Database**: Oracle Autonomous Database, all timestamps `TIMESTAMP WITH TIME ZONE` _(R18.1)_.
- **Ollama**: called server-side from the ORDS/PL/SQL handler (an outbound HTTP call from the database) for the review summary, with a graceful fallback _(R2.11–2.12)_.

### Cross-cutting decisions

| Concern | Decision | Requirement |
|---|---|---|
| Identity resolution | ORDS handler resolves the request principal to a single `CurrentUser` (id, display name, roles, team memberships, timezone). Authentication source is IDCS; the seeded-user testing login is a local-testing convenience resolved through the same abstraction. Feature code only ever reads `CurrentUser`. | R1.3, R1.7, R22.4 |
...
| Passwords | Hashed with a one-way hash (the specific KDF is an implementation detail). Never returned or logged. Seeded testing accounts = "password1" hashed. | R1.5–1.6, R21.6 |
| Dates | Stored `TIMESTAMP WITH TIME ZONE`; API returns ISO-8601 UTC; frontend renders in browser-local time; month buckets computed in the viewer's timezone. | R18 |
...
All tables use surrogate identity primary keys (`GENERATED ... AS IDENTITY`), `created_at`/`updated_at` of type `TIMESTAMP WITH TIME ZONE`, and are written through the API only. Boolean-style columns are modelled as `NUMBER(1)` with a `CHECK (col IN (0,1))` constraint (Oracle has no native BOOLEAN in tables pre-23c); status and other closed domains are modelled as `VARCHAR2` with a `CHECK` constraint (or a reference/lookup table) rather than a native enum type. JSON-bearing columns use Oracle `JSON`/`CLOB` with an `IS JSON` check constraint.
...
**`team`** _(R13, R20)_
- `id` PK (identity), `title`, `description`
- `team_leader_id` → `app_user.id`
- `is_closed` `NUMBER(1)` CHECK (0/1), default 0
...
- `id` PK (identity), `task_version_id` → `task_version.id`
- `data_point_id` → `data_point.id`
- `field_order`
- `is_mandatory` `NUMBER(1)` CHECK (0/1)
- `help_text_override` (nullable), `description_override` (nullable)
- `options_override` (nullable Oracle `JSON`/`CLOB` with `IS JSON` check, DROPDOWN only)
...
- `status` (`VARCHAR2` + CHECK constraint, see state machine)
...
- `id` PK (identity), `request_id`
- `author_id`
- `is_internal` `NUMBER(1)` CHECK (0/1)
- `is_system_generated` `NUMBER(1)` CHECK (0/1), default 0 _(R24.7)_ — set for the auto-generated change-type note documenting before/after fields and values; such notes are external (`is_internal=0`) and customer-visible.
- `body`, `created_at` (`TIMESTAMP WITH TIME ZONE`)
- The **change-type note** _(R24.7)_ and the **final closure note** _(R25.4)_ are both stored here as external notes (`is_internal=0`); the closure note is authored by the closing support member, the change-type note is system-generated.
...
- `started_at`, `ended_at` (both `TIMESTAMP WITH TIME ZONE`)
- `duration_minutes` (>= 1 when edited) _(R8.5)_

**`active_timer`** _(R4.6, R8.2, R8.8)_ — at most one open timer per (request, member)
- `id` PK (identity), `request_id`, `member_id`, `started_at` (`TIMESTAMP WITH TIME ZONE`)
- unique open-timer index so concurrency prompts can be resolved

### Audit & read-state

**`audit_entry`** _(R17)_ — column-level
- `id` PK (identity), `entity_type`, `entity_id`
- `field_name`, `old_value`, `new_value`
- `changed_by_id`, `changed_at` (`TIMESTAMP WITH TIME ZONE`)

**`request_last_seen`** _(R4.8, R5.2, R7.5–7.6)_ — drives the "Updated" indicator per user
- `request_id`, `user_id`, `last_seen_at` (`TIMESTAMP WITH TIME ZONE`)
- The "Updated" flag = request has a non-internal change after this user's `last_seen_at`.

### Status domain & state machine _(R9)_

States (a `VARCHAR2` domain constrained by a `CHECK` constraint, or a reference/lookup table): `NEW, TRIAGE, ACCEPTED, ASSIGNED, ACTIVE, PAUSED, BLOCKED, REJECTED, CANCELLED, COMPLETE`.
...
Base path `/api`, implemented as ORDS handlers. All endpoints require an authenticated principal except `POST /api/auth/login`. Authorisation is enforced per endpoint from the resolved roles. Responses are JSON; dates ISO-8601 UTC. All SQL uses bind variables (parameterised).
...
- `GET  /api/auth/users` — testing convenience: list for the seeded-user login drop-down _(R1.2)_.
...
- `POST /api/requests/{id}/notes` — add note (`is_internal=0` for raiser) _(R5.3)_.
...
All accept the client timezone (or use `CurrentUser.timezone`) for month bucketing. Month bucketing converts stored `TIMESTAMP WITH TIME ZONE` values into the viewer's timezone (using Oracle `FROM_TZ`/`AT TIME ZONE`) and buckets by month (`TRUNC(..., 'MM')` or `TO_CHAR(..., 'YYYY-MM')`).
...
- Called **server-side** from `POST /api/review/summary`, which is an ORDS/PL/SQL handler making an outbound HTTP call from the database (keeps the model endpoint off the browser).
...
1. **Backend runtime** — the backend API is implemented as ORDS REST endpoints (an ORDS-enabled schema exposing RESTful services / PL/SQL handlers) running inside the Autonomous Database. The frontend depends only on the documented REST contract, so the handler implementation can evolve without a redesign.
2. **Authentication and session** — authentication is delegated to IDCS and resolved to a single `CurrentUser` abstraction; the seeded-user testing login is a local-testing convenience resolved through the same abstraction. Feature code reads only `CurrentUser`.
