> Oracle target variant. This document mirrors `requirements.md` but targets Oracle Autonomous Database + ORDS + IDCS as the sole platform. The non-Oracle original remains authoritative for the current build.
...
The application is a browser-based JavaScript/Angular frontend backed by an Oracle Autonomous Database (ADB) data store, following the `ui-foundations` steering specification. The backend API is implemented as Oracle REST Data Services (ORDS) endpoints running inside the Autonomous Database, the built static files are served via ORDS, and authentication is delegated to IDCS. Authentication is via IDCS; a seeded-user drop-down login is provided as a testing convenience for local testing against the ADB. The application treats the current user as a single resolved identity that can be sourced from either mechanism.
...
2. WHERE the seeded-user testing login is in use THE system SHALL present the username field as a drop-down listing each created person in the form "`<8-digit-username>` `<firstname>` (`<role>`) `<surname>`" (for example "11111111 Jason (Administrator) Hughes").
...
6. WHERE the seeded-user testing login is in use ALL user accounts SHALL have the password "password1" (stored hashed).
7. THE system SHALL resolve the current user's identity through a single abstraction so that the authentication source can be switched between IDCS and the seeded-user testing login without changing the rest of the application.
...
**User Story:** As a developer and deployer, I want the app to run on Oracle Autonomous Database with ORDS and IDCS, so that development and deployment share the single target platform.
...
2. THE system SHALL use the Oracle Autonomous Database (the `helpdesk` schema within the ADB) as its backend data store.
3. THE system SHALL implement its backend API as Oracle REST Data Services (ORDS) endpoints running inside the Autonomous Database, and SHALL serve the built Angular static files via ORDS from the Autonomous Database.
4. THE system SHALL keep the frontend and its data-access and identity concerns loosely coupled from the ORDS/PL/SQL implementation and IDCS authentication so that the frontend depends only on the documented REST contract and the single resolved-identity abstraction.
